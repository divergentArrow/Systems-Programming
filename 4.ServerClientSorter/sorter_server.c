...
gcc serverX.c -lpthread -o serverX
./serverX -p 8080

you might have to switch to 8081 sometimes


Second: 
gcc clientTony.c -lpthread -o clientTony
./clientTony -c duration -h 127.0.0.1 -p 8080 
...
//====== Project 1 : Old Unedited Methods =======
...
void oldmain (char** csvArray, int numOfCsvRows){
	...
	*/
	...
	//pthread_mutex_lock(&lockOldMainEnqueue);
		//printf("  OM: Queue Amt: %d\n", gloVars.queueAmt);
		//printf("  OM: gloNumOfColumns: %d\n", gloVars.gloNumOfColumns);
		largeEnqueue(movie);
		gloVars.queueAmt++;
		//printf("  OM: Queue Amt: %d\n", gloVars.queueAmt);
	//pthread_mutex_unlock(&lockOldMainEnqueue);
	
		//printf("  OM: TEST OUT: [%s]\n", movie[3].data[1]);
...
// Tony/Justin Str_replace method
...
// ======= Recieves Lines => Calls Old main for struct ==> Stores into Queue
void *connection_handler(void *socket_desc){
    ...
    //print all items of queue back to client (dump?).
    
    //call joinCSVs (joins and sorts all csvs until there is one BIG csv left)
...
int main(int argc , char *argv[]){
	...
	//Column name(s) are a MUST. so kill program without mercy if it doesn't exist.
	if (flagp ==0){
	...
	//Part 1b: initialize locks so that we can lock areas down just like how the FCC + Ajit Pai wants to lock our internet down in favour
	//         of large businesses and pretty much for money, which utterly screws over us consumers. smh. so disappointed - Dec 14 2017
