...
char * moddir(char* output,char *input){
    ...
    char * dummy = (char *)malloc((inputlen+1)*sizeof(char));//directly using input may modify it...
    strcpy(dummy,input);
    ...
    int count=0;
    char dirpath[64]={'.','/','\0','\0'};
    for (i=0;i<inputlen;i++){                               /* this loop counts amount of 
                                                                words in input by counting slashes except ones
                                                                possibly at beginning and end. then adds one because it's off
                                                                by one 
                                                            */
    ...
    if (input[0]=='/'){
        ...
        //find closest dir that is shared by input
        dir=opendir(dirpath);//initial dir is working dir
        while(1){   
        ...
        //relative!!!
        if (input[0]!='.'){//dotless relative dir
            dirpath[0]='.';
            dirpath[1]='/';
            for (i=2;i<64;i++){
                dirpath[i]='\0';
...
void * encode(Parameters * data){
    ...
    char * buffer;              //buffer to hold each line
    size_t bufsize = 2000;      //size of buffer
    ...
    for(i = 1; i <= (int)data->rows; i++){ 
		...
		/*********** NEW ***********/
		//send output row to server
		//printf("Sending Line: %d\n", i);
		write(sock, output_row, strlen(output_row));

		//wait for server message to stay synchronized
		//printf("Recieving Confirmation: %d\n", i);
    ...
    //printf("ENCODE COMPLETE RETURNING NOW\n");
    pthread_exit(NULL);
    //return NULL;
    return 0;
}
...
int main(int argc, char** argv){
    char * name = malloc(200*sizeof(char));             //starting point for directory traversal
    char * output_name = malloc(200*sizeof(char));      //path to print file destination                                //item in current directory
    //char * column;
    int i;                                              //used for loops
    char * path = malloc(1000*sizeof(char));            //current path in traversal
    ...
    //send total # of lines in all csvs
    write(sock, total_lines, strlen(total_lines));
    //printf("Total lines sent\n");



    //read back server message to stay synchronized
    read_size = recv(sock, recvd, 2000,0);
	//printf("[S]: %s]\n", recvd);
	recvd[read_size] = '\0';
	//clear recvd
    ...
    traverse(path); //now writes lines one-by-one to sock
    ...
    //send dump request in form of column index
	...
	//clear recvd
	memset(recvd, 0, 2000);
    //printf("READY FOR MY PACKAGE SHIPMENT AAAAAAAAAAA \n");
    ...
    //receive and print lines to file one by one
